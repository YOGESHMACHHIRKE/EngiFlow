...
export const ActiveViewers: React.FC<ActiveViewersProps> = ({ users }) => {
  ...
  // Fix: Explicitly type the Map to ensure uniqueUsers is inferred correctly as User[]
  const uniqueUsers = Array.from(new Map<string, User>(users.map(u => [u.email, u])).values());
