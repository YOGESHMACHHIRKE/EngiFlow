...
const NavItem: React.FC<{
  ...
  isDanger?: boolean;
  badge?: string;
}> = ({ icon, label, isActive, onClick, isDanger = false, badge }) => {
  return (
    <li
      onClick={onClick}
    ...
    >
      ...
      {badge && !isActive && (
        <span className="text-[10px] font-black bg-amber-500 text-white px-1.5 py-0.5 rounded uppercase tracking-tighter animate-pulse">
            {badge}
        </span>
      )}
    </li>
        ...
        <nav className="p-4">
          <ul>
            ...
            <NavItem
              ...
              onClick={() => handleViewChange('use-cases')}
              badge="NEW"
            />
          </ul>
        </nav>
